	$(function() {	//flexigrid表格设置
	    $("#flex").flexigrid({
		        url : 'adminList.action',	//ajax方式对应的url地址
	...
	function test(com, grid) {
		 switch (com) {
			 ...
			 case '添加' :
				...
				$('#form input[name="admin.username"]').removeAttr("readonly");//为name为username 的标签去除只读属性
				$('#form input[name="admin.username"]').css("border-width","2px");	//设置边框
				action="adminAdd.action";
			 ...
			 case '修改' :
			    ...
			    $('#form input[name="admin.username"]').val(data[1]).attr("readonly","readonly");
			    $('#form input[name="admin.password"]').val(data[2]);
			    $('#form input[name="admin.password"]')[0].focus();	//密码框获得焦点
			    $('#form input[name="admin.username"]').css("border-width","0");	//设置边框
			    action="adminUpdate.action";
			    $("#usermsg").jqmShow();
			    break;
